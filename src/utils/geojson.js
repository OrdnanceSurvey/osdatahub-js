...
const geojson = {
  from: (input) => {
    try {
      let output = input.features[0] || input;
      for (let i = 0; i < output.geometry.coordinates.length; i++) {
        ...
        for (let j = 0; j < output.geometry.coordinates[i].length; j++) {
          ...
          [
            output.geometry.coordinates[i][j][0],
            output.geometry.coordinates[i][j][1],
          ] = [
            output.geometry.coordinates[i][j][1],
            output.geometry.coordinates[i][j][0],
          ];
    ...
    if (!input.results) {
      ...
      for (let i = 0; i < input.results.length; i++) {
        output.features.push({
          ...
          geometry: {
            type: "Point",
            coordinates: [input.results[i].DPA.LNG, input.results[i].DPA.LAT],
          },
        });
      }
        ...
        output.features.push({
          ...
          properties: {
            ...
            MOST_DETAIL_VIEW_RES:
              input.results[i].GAZETTEER_ENTRY.MOST_DETAIL_VIEW_RES,
            LEAST_DETAIL_VIEW_RES:
              input.results[i].GAZETTEER_ENTRY.LEAST_DETAIL_VIEW_RES,
            ...
            POSTCODE_DISTRICT:
              input.results[i].GAZETTEER_ENTRY.POSTCODE_DISTRICT,
            POSTCODE_DISTRICT_URI:
              input.results[i].GAZETTEER_ENTRY.POSTCODE_DISTRICT_URI,
            POPULATED_PLACE: input.results[i].GAZETTEER_ENTRY.POPULATED_PLACE,
            POPULATED_PLACE_URI:
              input.results[i].GAZETTEER_ENTRY.POPULATED_PLACE_URI,
            POPULATED_PLACE_TYPE:
              input.results[i].GAZETTEER_ENTRY.POPULATED_PLACE_TYPE,
            DISTRICT_BOROUGH: input.results[i].GAZETTEER_ENTRY.DISTRICT_BOROUGH,
            DISTRICT_BOROUGH_URI:
              input.results[i].GAZETTEER_ENTRY.DISTRICT_BOROUGH_URI,
            DISTRICT_BOROUGH_TYPE:
              input.results[i].GAZETTEER_ENTRY.DISTRICT_BOROUGH_TYPE,
            ...
            COUNTY_UNITARY_TYPE:
              input.results[i].GAZETTEER_ENTRY.COUNTY_UNITARY_TYPE,
            ...
            RELATED_SPATIAL_OBJECT:
              input.results[i].GAZETTEER_ENTRY.RELATED_SPATIAL_OBJECT,
          },
          geometry: {
            type: "Point",
            coordinates: [
